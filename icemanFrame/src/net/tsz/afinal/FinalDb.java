...
public class FinalDb {
	...
	public static final int CopyDb = 1;
	...
	private FinalDb(DaoConfig config) {
		...
		} else if (config.getTargetDirectory() != null && config.getTargetDirectory().trim().length() > 0 && config.type != 1) {
	...
	public static void learDBCache() {
		daoMap.clear();
	}
	/**
	 * 创建FinalDb
	...
	public static class DaoConfig {
		...
		private String mDbName = "yangtzedu.db"; // 数据库名字
		private int dbVersion = 1; // 数据库版本
		private boolean debug = true; // 是否是调试模式（调试模式 增删改查的时候显示SQL语句）
		private DbUpdateListener dbUpdateListener;
		// add
		private int type = 0; // 判断是否创建新的数据库||是用已经存在的数据库 1为是用旧数据库 0为创建新的数据库
