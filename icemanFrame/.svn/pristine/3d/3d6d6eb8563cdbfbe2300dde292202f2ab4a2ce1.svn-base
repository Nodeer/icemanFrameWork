...
public class WheelView extends View {
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			...
			itemsPaint.setTextSize(DeviceUtil.getPixelFromDip(TEXT_SIZE));
		}

		if (valuePaint == null) {
			valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.FAKE_BOLD_TEXT_FLAG | Paint.DITHER_FLAG);
			// valuePaint.density = getResources().getDisplayMetrics().density;
			valuePaint.setTextSize(DeviceUtil.getPixelFromDip(TEXT_SIZE));
	...
	private int getDesiredHeight(Layout layout) {
		...
		int desired = getItemHeight() * visibleItems - DeviceUtil.getPixelFromDip(ITEM_OFFSET) * 2 - ADDITIONAL_ITEM_HEIGHT;
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			float textWidth = FloatMath.ceil(Layout.getDesiredWidth("零", itemsPaint));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (itemsWidth > 0) {
			...
			canvas.translate(PADDING, -DeviceUtil.getPixelFromDip(ITEM_OFFSET));
			drawItems(canvas);
			drawValue(canvas);
			canvas.restore();
		}
